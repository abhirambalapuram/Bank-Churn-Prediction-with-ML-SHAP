...
# A couple of helpful distributions
...
# Works across sklearn versions (sparse_output vs sparse)
...
numeric_pipe = Pipeline(steps=[
    ...
    ("scaler", StandardScaler(with_mean=False))  # keep False to be sparse-friendly
])
...
# Helper to fetch transformed feature names (requires a FITTED preprocessor)
...
# Extract base key (e.g., "RF_SMOTE" -> "RF")
...
# Cell 13 — Faster hyperparameter tuning (RandomizedSearchCV on RF)
...
param_distributions = {
    ...
    # Subsample for speed without much loss; remove if you want full data per tree
...
rs = RandomizedSearchCV(
    ...
    n_iter=18,            # reduce to 10 if still slow; increase to 24 for more thorough search
    scoring="f1",
    cv=3,
    n_jobs=-1,
    verbose=1,
...
# Choose RF pipeline to interpret: tuned if available; else final best if RF; else a quick RF fit
...
# Cell 16 — Robust SHAP for RF pipeline (handles version quirks & dim mismatches)
...
# Choose the RF pipeline we’re explaining (from Cell 15 logic)
rf_pipe_used = rf_for_importance
model = rf_pipe_used.named_steps["clf"]

# Use the EXACT fitted preprocessor inside this pipeline
preproc_used = rf_pipe_used.named_steps["pre"]

# Transform train/test with that preprocessor
...
# Recompute feature names from THIS fitted preprocessor (not a separately-fitted one)
...
# Try TreeExplainer first; fall back to generic Explainer if needed
try:
...
# Predict churn probability for first 5 rows from test set
